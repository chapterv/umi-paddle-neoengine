...
class BatchDOC(Page):
    ...
    def msnDocs(self, docs, argd):
        if self._msnID or self._queuedDocs:
            # 工人假死时 UI 状态可能仍占着；先尝试恢复再拒绝/重试由用户点停止
            if MissionDOC.isWorkerBusy() or self._msnID:
                logger.warning(
                    "msnDocs 拒绝：有任务进行中 "
                    f"msnID={self._msnID!r} queued={len(self._queuedDocs)} "
                    f"docBusy={MissionDOC.isWorkerBusy()}"
                )
            return "[Error] 有任务进行中，不允许提交新任务。请先点停止；若仍无法开始，再点一次停止以强制恢复。"
    ...
    def msnStop(self):  # 停止当前所有任务
        # 1) 先清本地队列/ID，避免 forceRecover→onEnd 误触发 _runNewDoc
        # 2) stop + forceRecover：杀 OCR 引擎、作废工人，解除 addMissionWait 假死
        mid = self._msnID
        self._msnID = ""
        self._pauseMsnID = ""
        self._queuedDocs = []
        ...
        try:
            if mid:
                MissionDOC.stopMissionList(mid)
        except Exception:
            logger.error("BatchDOC.msnStop stopMissionList 失败。", exc_info=True)
        try:
            MissionDOC.forceRecover(f"BatchDOC.msnStop mid={mid!r}")
        except Exception:
            logger.error("BatchDOC.msnStop forceRecover 失败。", exc_info=True)
