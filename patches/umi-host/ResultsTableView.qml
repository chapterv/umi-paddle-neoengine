...
Item {
    ...
    function addOcrResult(res) {
        ...
        switch(res.code){
            case 100: // 成功
                status_ = "text"
                // 表格结构优先：用 cells 生成 TSV，复制/导出到 Excel 才能分列
                // （data 仍保留原始 box 供左侧叠框预览，不在此改）
                if(res.table && res.table.cells && res.table.cells.length > 0) {
                    const cells = res.table.cells
                    for(let r=0; r<cells.length; r++) {
                        const row = cells[r] || []
                        for(let c=0; c<row.length; c++) {
                            resText += (row[c]===undefined || row[c]===null) ? "" : String(row[c])
                            if(c < row.length-1) resText += "\t"
                        }
                        if(r < cells.length-1) resText += "\n"
                    }
                } else {
        ...
        // 补充空白标题：顺序 = 耗时 | 置信度 | 推理模块
        // （推理模块作为独立尾段，不再夹在耗时与置信度之间）
