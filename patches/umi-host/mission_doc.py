...
# 需大于 PPOCR_api 单次读超时（120s），并留引擎重启余量。
DOC_OCR_WAIT_TIMEOUT = 180
...
class _MissionDocClass(Mission):
    ...
    def _onStopRequested(self, msnIDs):
        # DOC 工人常阻塞在 MissionOCR.addMissionWait；必须连带停 OCR 并杀引擎才能返回
        logger.warning("MissionDOC stop → 中断嵌套 MissionOCR / 引擎")
        try:
            MissionOCR.stopAllMissions()
        except Exception:
            logger.error("MissionDOC 停止嵌套 OCR 失败。", exc_info=True)
        try:
            MissionOCR.interruptEngine("MissionDOC stop")
        except Exception:
            logger.error("MissionDOC 中断 OCR 引擎失败。", exc_info=True)

    def forceRecover(self, reason=""):
        """停止后批量仍起不来时调用：清空 DOC + OCR 工人槽位。"""
        reason = reason or "MissionDOC.forceRecover"
        try:
            MissionOCR.forceRecover(reason)
        except Exception:
            logger.error("嵌套 MissionOCR.forceRecover 失败。", exc_info=True)
        super().forceRecover(reason)
    ...
    def msnTask(self, msnInfo, pno):  # 执行msn。pno为当前页数（0-based）
        page_ui = pno + 1  # UI 页码从 1 起
        t_page0 = time.time()
        logger.info(
            f"[DOC] P{page_ui} start path={msnInfo.get('path', '')!r} "
            f"mode={msnInfo.get('argd', {}).get('doc.extractionMode')}"
        )
        # 若队列已被 stop，尽快返回，避免继续渲染/OCR
        if msnInfo.get("state") == "stop":
            logger.warning(f"[DOC] P{page_ui} skipped (state=stop)")
            return {"code": 803, "data": "[Warning] Task stop."}
        ...
        if extractionMode == "fullPage":  # 模式：整页强制OCR
            ...
            try:
                ...
                t_pm0 = time.time()
                logger.info(
                    f"[DOC] P{page_ui} pixmap begin size={w:.0f}x{h:.0f} zoom={zoom:.3f}"
                )
                p = page.get_pixmap(matrix=matrix)
                img_bytes = p.tobytes("png")
                logger.info(
                    f"[DOC] P{page_ui} pixmap done "
                    f"{time.time() - t_pm0:.2f}s bytes={len(img_bytes)}"
                )
                scale = 1 / zoom
                imgs.append(
                    {
                ...
                logger.error(
                    f"[DOC] P{page_ui} pixmap/render 失败",
                    exc_info=True,
                    stack_info=True,
                )
                errMsg += f"[Error] page render: {type(e).__name__}: {e}\n"
        else:
        ...
        if imgs and msnInfo.get("state") != "stop":
            ...
            t_ocr0 = time.time()
            logger.info(
                f"[DOC] P{page_ui} OCR wait begin n_img={len(imgs)} "
                f"timeout={DOC_OCR_WAIT_TIMEOUT}s"
            )
            try:
                ocrList = MissionOCR.addMissionWait(
                    ocrArgd, imgs, timeout=DOC_OCR_WAIT_TIMEOUT
                )
            except Exception as e:
                logger.error(
                    f"[DOC] P{page_ui} OCR wait 异常",
                    exc_info=True,
                    stack_info=True,
                )
                ocrList = []
                errMsg += f"[Error] OCR wait: {type(e).__name__}: {e}\n"
            logger.info(
                f"[DOC] P{page_ui} OCR wait done {time.time() - t_ocr0:.2f}s "
                f"n_res={len(ocrList)}"
            )
            # 整理OCR结果
            for o in ocrList:
        ...
        elif imgs and msnInfo.get("state") == "stop":
            logger.warning(f"[DOC] P{page_ui} OCR skipped (state=stop)")
        ...
        # 其他tbpu
        # docs/11：脏空白页等可能让 tbpu 抛错；捕获后本页记错继续，避免拖死整本
        if msnInfo["tbpu"] and tbs:
            for tbpu in msnInfo["tbpu"]:
                try:
                    ...
                    logger.error(
                        f"文档 tbpu 异常 P{pno} ({getattr(tbpu, 'tbpuName', type(tbpu).__name__)})",
                        exc_info=True,
                        stack_info=True,
                    )
                    errMsg += f"[Error] tbpu: {type(e).__name__}: {e}\n"
                    break
        ...
        logger.info(
            f"[DOC] P{page_ui} end code={resDict.get('code')} "
            f"n_tb={len(tbs) if tbs else 0} total={time.time() - t_page0:.2f}s"
        )
        return resDict
