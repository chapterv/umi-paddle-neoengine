...
def get_gpu_mem_mb():
    """Return device-wide used VRAM when NVML is available, otherwise -1."""
    global _NVML
    if DEVICE != "gpu":
        return -1
    try:
        ...
        return round(_NVML.nvmlDeviceGetMemoryInfo(handle).used / 1048576, 1)
    except Exception:
        return -1
...
def main():
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if img is None:
                ...
                if _do_pre:
                    ...
                    run_img = tmp_padded
                preprocess_ms = (time.perf_counter() - t_preprocess0) * 1000
                ...
                try:
                    ...
                    convert_ms = (time.perf_counter() - t_convert0) * 1000
                    ...
                    except Exception as punctuation_exc:
                        sys.stderr.write(
                    ...
                    if task == "table" and TABLE_STRUCTURE_ENABLED:
                        ...
                        table_ms = (time.perf_counter() - t_table0) * 1000
                finally:
                    # 仅清理 V6 补丁产生的临时文件
        ...
        t_json_encode0 = time.perf_counter()
        json_payload = json.dumps(out, ensure_ascii=True)
        json_encode_ms = (time.perf_counter() - t_json_encode0) * 1000
